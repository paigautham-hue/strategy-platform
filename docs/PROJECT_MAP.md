...
| Analysis-run history | `server/services/analysis-runs.ts`, drizzle `analysis_run`, `client/src/components/AnalysisHistory.tsx` | Every reasoning result persisted (best-effort) + listable per company + print/PDF export. Kinds now include persona consults, playbook drafts, and pattern-mining runs (migration `0005`). Pattern Mining → one-click "Draft playbook"; Distillation → GP-only Publish that re-verifies the gate server-side and writes the anonymized pattern into the GLOBAL memory layer. |
