...
# RESOLUTION LOG — 2026-07-08 (Claude Code)

Verified after fixes: `tsc` clean · production build clean · 573/589 tests pass.

| ID | Status | How it was fixed |
|----|--------|------------------|
| B1 | ✅ Fixed | Raw-key path now ONLY on explicit `GEMINI_USE_EPHEMERAL_TOKENS=false` opt-in; a failed mint throws — the key never ships as a fallback (`geminiRealtime.ts`). |
| H1 | ✅ Fixed | `enforceCompanyScope` middleware on `protectedProcedure` (`server/_core/trpc.ts`) — ANY procedure whose input carries a numeric `companyId` is checked against role + `assignedCompanyIds` before the resolver runs. gp/operator/admin procedures extend it, so the whole API surface is covered structurally. |
| H2/H11 | ✅ Fixed | `supersedeMemory` now loads the old row scoped to tenant+company and rejects missing/already-invalidated items — no IDOR, no silent bi-temporal break. |
| H3 | ✅ Fixed | `purgeMemoryItem` requires and enforces `companyId`. |
| H4 | ✅ Fixed | Voice system prompt description + `lookup_company`/`lookup_memory`/`lookup_predictions` results are redacted; memory/prediction voice reads audit-logged. |
| H5 | ✅ Fixed | `vision.generate` prompt redacted before the image provider. |
| H6 | ✅ Fixed | MCP `lookup_memory` appends a confidential-tier audit entry. |
| H7 | ✅ Fixed | `prediction.list` / `listOpen` audit-logged. |
| H8 | ✅ Fixed | `memory.writeLayer(global)` is GP/admin-only. Global `queryLayer` stays readable by design — the layer holds framework canon + ALREADY-ANONYMIZED published patterns. |
| H9 | ✅ Fixed | Export now AES-256-GCM (scrypt-derived per-company key, random IV, auth tag; format `CAIRNv1 | iv | tag | ciphertext`). |
| C1 | 🟡 Partial | Research-mesh synthesis claims now extracted + recorded to the ledger (≤5/run, `model: research-mesh`). Extending to frameworks/options/personas is follow-up — each needs a claim-extraction pass and cost review. |
| C2 | ✅ Fixed | Empty catches replaced with loud `console.error` on every ledger write path. |
| C3 | ✅ Fixed | Already-resolved check in `closePrediction` + `UNIQUE(outcome.predictionId)` (migration `0006`). |
| H10 | ✅ Fixed | `writeMemory` returns the existing row on idempotency-key hit (no duplicate, no re-spend). |
| H12 | ✅ Fixed | `logCall` increments the daily-spend cache, so parallel fan-out can no longer blow through the cap within the TTL. |
| H13 | ✅ Fixed | Red-team fails CLOSED (`survivedReview: false` + explicit fatal flag) when the review cannot run. |
| F1 | ✅ Fixed | Get-started steps 2–3 disabled until a company exists. |
| F2 | ✅ Fixed | `analysisRuns.list` invalidated on success in all 8 mutation pages. |
| F3 | ✅ Fixed | `AnalysisHistory` gets skeleton/empty/error states (`showEmpty` on `/history`). |
| F5/F6 | ✅ Fixed | Editing the pattern text resets both preview and publish state. |
| F7 | ✅ Fixed | UI caps cross-co selection at 6 with a toast. |
| F8 | ✅ Fixed | Toast uses a `sonner` action button instead of a nested anchor. |
| F10 | ✅ Fixed | `GpOnlyGate` on Distillation, Export, Synergy Scout pages. |
| F4, F9, M1–M7, L1–L5 | ☐ Open | Deferred — tracked as follow-up; none block the demo. M2 (daily cap fails open on DB error) is deliberate: a DB hiccup must not take down all AI calls. |
