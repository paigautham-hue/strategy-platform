...
- **All three vendors wired direct** (same never-break fallback): `server/_core/google.ts` (Gemini generateContent — `worker` + `creative` tiers now run direct Gemini 2.5 Flash) and `server/_core/openaiChat.ts` (OpenAI chat, available as a provider; OpenAI also powers embeddings + opt-in realtime voice). The dispatcher routes `anthropic`/`google`/`openai` from models.yaml; a missing key or failure on any of them degrades to forge.
- **Self-explanatory UI**: every sidebar item now carries a one-line plain-language description (hover tooltip); all routed pages already have a subtitle; manual + FAQ updated (Ask Cairn, History/export, which-models-when).
- **Verification**: `tsc` clean · production build clean · **573/589 tests pass, 16 skipped, 0 fail** (+32 new provider tests: payload construction, refusal/fence handling, fallback behavior per vendor, pricing, tier config, planner envelope). Live smoke test per vendor pending keys in Manus env — until set, tiers degrade to forge by design; verify post-deploy via `llm_call_log.model`.
